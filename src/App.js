...
import DriverDashboard from './components/Views/Driver/DriverDashboard';
...
function App() {
  ...
  const [isDriver, setIsDriver] = useState(false); // New state for driver login
  ...
  const handleLogout = () => {
    ...
    setIsLoggedIn(false);
    setIsAdmin(false);
    setIsDriver(false); // Reset driver state on logout
  };
